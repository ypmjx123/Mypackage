...
#' result <- path_mut_visual(SNV = SNV_data,
...
#'                            test = "wilcox.test")
#' print(result)
path_mut_visual<-function(SNV,gene,Gene_group,Type,pathway_gene_data,
                          tumor,color,test="wilcox.test"){
  library(dplyr)
  library(ggplot2)
  library(tidyr)
  library(ggpubr)
  ...
  p<-ggplot(data = result, mapping = aes(x = Pathway, y = Mutation_rate, fill = Types)) +
    ...
    labs(x = "Tumor signaling pathways", y = "Mutation rate(%)",fill =paste(gene,"Group"),
         title = paste("Frequency of tumor signaling  pathway alterations in",tumor)) +
    coord_flip() +
